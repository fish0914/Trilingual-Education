...
<body class="bg-slate-50 text-slate-800 font-sans min-h-screen flex flex-col">
    ...
    <header class="bg-white shadow-sm sticky top-0 z-40 border-b border-slate-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl font-bold text-slate-900 tracking-tight">三語計畫成果展</h1>
                    <p class="text-xs text-slate-500 font-medium">SDGs科技創作 x 資訊專題</p>
                </div>
            </div>
            ...
            <nav class="flex space-x-1 bg-slate-100 p-1 rounded-lg">
                <button onclick="switchTab('gallery')" id="btn-tab-gallery" class="px-4 py-2 text-sm font-medium rounded-md transition-all bg-white text-blue-600 shadow-sm flex items-center gap-2">
                    <i data-lucide="layout" class="w-4 h-4"></i> 成果展示
                </button>
                <button onclick="switchTab('admin')" id="btn-tab-admin" class="px-4 py-2 text-sm font-medium rounded-md transition-all text-slate-600 hover:text-slate-900 flex items-center gap-2">
                    <i data-lucide="upload" class="w-4 h-4"></i> 資料上傳
                </button>
            </nav>
        </div>
    </header>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 w-full">
        ...
        <div id="view-gallery" class="block">
            ...
            <div id="gallery-empty" class="hidden text-center py-20 bg-white rounded-2xl border-2 border-dashed border-slate-200">
                ...
                <h3 class="mt-2 text-sm font-semibold text-slate-900">目前尚無成果</h3>
                <p class="mt-1 text-sm text-slate-500">請切換至「資料上傳」頁面新增學生作品。</p>
            </div>
        </div>
        ...
        <div id="view-admin" class="hidden">
            ...
            <div id="admin-login" class="flex flex-col items-center justify-center py-20 bg-white rounded-xl shadow-sm border border-slate-200 max-w-md mx-auto">
                ...
                <h3 class="text-lg font-bold text-slate-800 mb-2">管理員權限鎖定</h3>
                <p class="text-slate-500 mb-6 text-sm">請輸入管理密碼以進行資料上傳或刪除。</p>
                <form onsubmit="handleAdminLogin(event)" class="flex gap-2 w-full px-8">
                    <input type="password" id="admin-password" class="flex-1 px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm" placeholder="輸入密碼..." autofocus>
                    <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 text-sm">解鎖</button>
                </form>
            </div>
            ...
            <div id="admin-dashboard" class="hidden grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-200 sticky top-24">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-lg font-bold text-slate-900 flex items-center gap-2">
                                ...
                                新增 <span id="admin-course-type-text">SDGs</span> 作品
                            </h3>
                        ...
                        <form onsubmit="handleUpload(event)" class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">作品標題 *</label>
                                <input required type="text" id="input-title" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="例如：智慧垃圾桶設計">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">學生姓名/組員 *</label>
                                <input required type="text" id="input-student" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="例如：王小明、李小華">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">作品簡介</label>
                                <textarea rows="3" id="input-desc" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="請簡述創作理念..."></textarea>
                            </div>

                            <div class="p-4 bg-slate-50 rounded-lg border border-slate-200 space-y-4">
                                <p class="text-xs text-slate-500 font-bold uppercase tracking-wider">資源設定</p>
                                
                                <div>
                                    <label class="block text-xs font-medium text-slate-600 mb-1 flex items-center gap-1">
                                        <i data-lucide="youtube" class="w-3 h-3"></i> 影片連結 (URL 或 嵌入代碼)
                                    </label>
                                    <input type="text" id="input-video" oninput="updatePreview()" class="w-full px-3 py-2 text-sm border border-slate-300 rounded-md focus:ring-1 focus:ring-blue-500 font-mono" placeholder="貼上 YouTube 網址 或 <iframe...> 原始碼">
                                    <div id="video-preview-container" class="mt-3 bg-black rounded-lg overflow-hidden aspect-video relative hidden">
                                        ...
                                        <div class="absolute top-2 right-2 bg-black/70 text-white text-[10px] px-2 py-1 rounded pointer-events-none">即時預覽</div>
                                    </div>
                                    <p id="video-error-msg" class="hidden mt-2 text-[10px] text-red-500 flex items-center gap-1">
                                        <i data-lucide="alert-triangle" class="w-3 h-3"></i> 無法識別影片 ID，請檢查內容
                                    </p>
                                </div>

                                <div>
                                    <label class="block text-xs font-medium text-slate-600 mb-2 flex items-center gap-1">
                                        <i data-lucide="file-text" class="w-3 h-3"></i> 簡報連結 (Google/Canva)
                                    </label>
                                    <input type="url" id="input-slides" class="w-full px-3 py-2 text-sm border border-slate-300 rounded-md focus:ring-1 focus:ring-blue-500" placeholder="Google Slides / Canva 連結">
                                    <p class="text-[10px] text-slate-400 mt-1">* Canva 請使用「分享」&gt;「嵌入」連結效果最佳</p>
                                </div>
                            </div>
                            ...
                            <button type="submit" id="btn-submit" class="w-full bg-blue-600 text-white py-2.5 rounded-lg font-medium hover:bg-blue-700 transition-colors flex justify-center items-center gap-2">
                                <i data-lucide="check-circle-2" class="w-4 h-4"></i> 確認發布
                            </button>
                        </form>
                    </div>
                </div>
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                        ...
                        <div id="admin-empty" class="p-8 text-center text-slate-500 hidden">此分類尚未上傳資料</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script type="module">
        ...
        window.handleUpload = async (e) => {
            ...
            btn.innerHTML = `<i data-lucide="loader-2" class="animate-spin w-4 h-4"></i> 處理中...`;
            lucide.createIcons();
            btn.disabled = true;
        ...
        window.openModal = (id, type) => {
            ...
            authorEl.textContent = `作者：${project.studentName}`;
                    ...
                    <div class="w-full h-full bg-black flex flex-col">
                        ...
                        <div class="bg-slate-900 border-t border-slate-700 text-white p-4 text-center text-sm flex flex-col items-center gap-2">
                             <a href="${project.videoUrl}" target="_blank" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-full transition-colors flex items-center gap-2 font-bold text-xs">
                                <i data-lucide="external-link" class="w-4 h-4"></i> 前往 YouTube 觀看
                            </a>
                        </div>
                    </div>
                `;
                    ...
                    <div class="w-full h-full bg-slate-100 flex flex-col">
                        ...
                        <div class="bg-slate-200 text-slate-600 p-2 text-center text-xs flex justify-center gap-2">
                            <a href="${project.slidesUrl}" target="_blank" class="text-blue-600 hover:text-blue-800 underline flex items-center gap-1">
                                <i data-lucide="external-link" class="w-3 h-3"></i> 在新視窗開啟
                            </a>
                        </div>
                    </div>
                `;
        ...
        function renderApp() {
                ...
                galleryGrid.innerHTML = filtered.map(p => `
                    <div class="group bg-white rounded-xl shadow-sm border border-slate-200 hover:shadow-md transition-shadow overflow-hidden flex flex-col h-full">
                        ...
                        <div class="p-6 flex-1 flex flex-col">
                            <p class="text-slate-600 text-sm mb-6 flex-1 line-clamp-3">${escapeHtml(p.description || "無專案描述...")}</p>
                            <div class="grid grid-cols-2 gap-3 mt-auto">
                                <button onclick="openModal('${p.id}', 'video')" ${!p.videoUrl ? 'disabled' : ''} class="flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${p.videoUrl ? 'bg-red-50 text-red-700 hover:bg-red-100' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}">
                                    <i data-lucide="monitor-play" class="w-4 h-4"></i> 觀看影片
                                </button>
                                <button onclick="openModal('${p.id}', 'slides')" ${!p.slidesUrl ? 'disabled' : ''} class="flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${p.slidesUrl ? 'bg-amber-50 text-amber-700 hover:bg-amber-100' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}">
                                    <i data-lucide="presentation" class="w-4 h-4"></i> 閱讀簡報
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            if (!isAdminLoggedIn) {
                ...
                document.getElementById('admin-course-type-text').textContent = currentFilter === 'sdgs' ? 'SDGs' : '資訊專題';
                        ...
                        <li class="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                            <div class="flex-1 min-w-0 pr-4">
                                ...
                                <div class="flex gap-2 mt-1">
                                    ${p.videoUrl ? '<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800">影片 OK</span>' : ''}
                                    ${p.slidesUrl ? '<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-100 text-amber-800">簡報(連結)</span>' : ''}
                                </div>
                            </div>
